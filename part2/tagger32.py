...
import utils42 as utils

I2W = utils.I2W
prefix_dict = utils.prefix_dict
suffix_dict = utils.suffix_dict
UNK = utils.UNK
END = utils.END
AFFIX_LENGTH = utils.AFFIX_LENGTH


def get_prefix(word):
    if word not in I2W:
        return prefix_dict[I2W[UNK]]
    word = I2W[word]
    if word == UNK or word == END or len(word) <= AFFIX_LENGTH:
        return prefix_dict[word]
    return prefix_dict[word[:AFFIX_LENGTH]]


def get_suffix(word):
    if word not in I2W:
        return suffix_dict[I2W[UNK]]
    word = I2W[word]
    if word == UNK or word == END or len(word) <= AFFIX_LENGTH:
        return suffix_dict[word]
    return suffix_dict[word[-AFFIX_LENGTH:]]
...
class Net(nn.Module):
    def __init__(self, d_in, d_out, lr, mode, iter_num, d_hid=64, embedding_dim=50, win_size=5):
        ...
        self.E_pre = nn.Embedding(d_in, embedding_dim)
        self.E_suff = nn.Embedding(d_in, embedding_dim)
    ...
    def forward(self, inputs):
        prefixes, suffixes = inputs.data.numpy().copy(), inputs.data.numpy().copy()
        prefixes, suffixes = prefixes.reshape(-1), suffixes.reshape(-1)

        for i, prefix in enumerate(prefixes):
            prefixes[i] = get_prefix(prefix)
        for i, suffix in enumerate(suffixes):
            suffixes[i] = get_suffix(suffix)

        prefixes, suffixes = torch.from_numpy(prefixes.reshape(inputs.data.shape)), torch.from_numpy(
            suffixes.reshape(inputs.data.shape))
        prefixes, suffixes = prefixes.type(torch.LongTensor), suffixes.type(torch.LongTensor)
        prefixes, suffixes = Variable(prefixes), Variable(suffixes)

        embeds = (self.embeddings(inputs) + self.E_pre(prefixes) + self.E_suff(suffixes)).view(
            (-1, self.window_size * self.emb_dim))
...
if __name__ == '__main__':
    ...
    if MODE == 'POS':
        utils.fill_data_structures(utils.train_pos_lines)
        ...
        utils.fill_data_structures(utils.train_ner_lines)
    ...
    print 'predict test'
    model.predict_test(TEST, 'test4.' + MODE.lower())
